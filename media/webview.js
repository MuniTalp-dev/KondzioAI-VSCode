(() => {
  ...
  function showHealth(value) {
    ...
    (value.items || []).forEach(item => { const icon = item.status === "ERROR" ? "✕" : item.status === "WARNING" ? "⚠" : "✓"; const name = document.createElement("div"); name.textContent = `${icon} ${item.name}`; const state = document.createElement("details"); state.innerHTML = `<summary class="${item.status}">${item.status}${item.status === "WARNING" && item.name === "Codex CLI" ? " — NIE ZAINSTALOWANY" : ""}</summary><span class="muted">${esc(item.version || item.detail)}</span>`; root.append(name, state); });
  }
  const checking = () => showHealth({ items: ["Orchestrator", "MCP", "SearXNG", "Ollama", "Qwen", "Aider", "Codex", "Git", ".NET"].map(name => ({ name, status: "CHECKING" })) });
  function showUpdate(value) { const presentation = presentVersionStatus(value); releaseUrl = value.releaseUrl; $("versionStatus").className = `status-chip ${presentation.tone}`; $("versionStatus").textContent = presentation.text; $("openRelease").classList.toggle("hidden", !presentation.showReleaseButton); $("installUpdate").classList.toggle("hidden", value.status !== "updateAvailable"); }
  ...
  window.addEventListener("message", event => {
    ...
    if (message.type === "warning") $("warning").textContent = message.value; if (message.type === "error") { $("error").textContent = message.value; $("installStatus").textContent = message.value; } if (message.type === "installState") $("installStatus").textContent = message.value; if (message.type === "installSuccess") { $("installStatus").textContent = message.value; $("installUpdate").classList.add("hidden"); $("reloadWindow").classList.remove("hidden"); } if (message.type === "busy") document.querySelector('[data-command="run"]').disabled = message.value; if (message.type === "pathSelected") $(message.key).value = message.value; if (message.type === "settings") Object.entries(message.value).forEach(([key, value]) => { $(key).value = value; }); if (message.type === "settingsSaved") $("settingsStatus").textContent = "Ustawienia zapisane.";
