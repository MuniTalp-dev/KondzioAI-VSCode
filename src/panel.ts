...
export class KondzioViewProvider implements vscode.WebviewViewProvider, vscode.Disposable {
  ...
  resolveWebviewView(view: vscode.WebviewView): void {
    ...
    view.webview.html = html(this.updates.installedVersion, this.settings());
  ...
  private async handle(message: PanelMessage): Promise<void> {
    ...
    try {
      switch (message.type) {
        case "clientInitialized": this.log("WebView client initialized"); break;
...
export function html(installedVersion = "", settings: Record<string, string> = pathDefaults): string {
  const nonce = Math.random().toString(36).slice(2);
  return `<!doctype html><html lang="pl"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; style-src 'nonce-${nonce}'; script-src 'nonce-${nonce}';"><style nonce="${nonce}">
  ...
  <details open><summary>WERSJA</summary><div class="card"><strong>Kondzio AI ${installedVersion}</strong><div id="versionStatus" class="status-chip" role="status" aria-live="polite">? NIE SPRAWDZONO</div></div><button id="openRelease" class="hidden" data-command="openRelease">OTWÓRZ WYDANIE</button></details>
