...
export function html(installedVersion = ""): string {
  ...
  <details open><summary>WERSJA</summary><div id="update" class="card"><strong>Kondzio AI</strong><div class="version-lines">Wersja zainstalowana: ${installedVersion}</div><div id="versionStatus" class="version-status progress" role="status" aria-live="polite">Nie sprawdzono aktualności oprogramowania.</div></div><div class="actions"><button id="updateCheck" type="button">SPRAWDŹ AKTUALIZACJĘ</button><button id="updateNow" type="button">SPRAWDŹ / OTWÓRZ WYDANIE</button><button id="updateLater" type="button">PÓŹNIEJ</button></div></details>
  ...
  const presentVersionStatus=${versionStatusPresentation.toString()};
  ...
  function update(v){const isChecking=v.status==='checking',presentation=presentVersionStatus(v);releaseUrl=v.releaseUrl;$('updateCheck').disabled=isChecking;$('updateNow').style.display=presentation.showReleaseButton?'block':'none';$('updateNow').disabled=!presentation.showReleaseButton;const lines=$('update').querySelector('.version-lines');lines.textContent='Wersja zainstalowana: '+esc(v.currentVersion)+'\nNajnowsza wersja: '+esc(v.latestVersion);const state=$('versionStatus');state.className='version-status '+presentation.tone;state.textContent=presentation.text}
  addEventListener('message',e=>{const m=e.data;if(m.type==='status')status(m.value);if(m.type==='history')history(m.value);if(m.type==='research')research(m.value);if(m.type==='health')health(m.value);if(m.type==='healthChecking')checking();if(m.type==='updateState')update(m.value);if(m.type==='updateDismissed')$('versionStatus').textContent='Aktualizację odłożono.';if(m.type==='warning')$('warning').textContent=m.value;if(m.type==='error')$('error').textContent=m.value;if(m.type==='busy')$('run').disabled=m.value;if(m.type==='preset'){if(m.autonomy)$('autonomy').value=String(m.autonomy);if(m.section==='research'){$('researchPanel').open=true;$('query').focus()}else $('prompt').focus();updateInfo()}});checking();
